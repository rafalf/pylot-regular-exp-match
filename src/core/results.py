...
def generate_results(dir, test_name):
    ...
    re_exp_load_time = regular_expressions_with_page_load(merged_log)
    ...
    reportwriter.write_regular_expression_match(fh, re_exp_load_time)
...
def regular_expressions_with_page_load(merged_log):
    elapsed_times = []
    stats_lists = [line.split(',') for line in merged_log]
    for stats in stats_lists:
        if stats[5] == '200':  # just concerned with valid responses
            elapsed_times.append([stats[1], stats[2], stats[4], stats[8], stats[11].rstrip()])
